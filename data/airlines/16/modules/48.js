...
casper.waitFor(function() {
	...
	casper.exit(130);});casper.waitFor(function() {
	return casper.evaluate(function() {
		return $('#arrival-airport').size() > 0;
	});
}, undefined, function(){
	casper.exit(131);});casper.waitFor(function() {
	return casper.evaluate(function() {
...
&& $('#flight-type-one-way-trip').val() === 'ONE_WAY' &&  $('#flight-type-one-way-trip').is(':not(:checked)');
	...
	casper.exit(132);});casper.waitFor(function() {
	return casper.evaluate(function() {
		return $('#departure-date').size() > 0 && $('#return-date').size() > 0 ;
	});
}, undefined, function(){
	casper.exit(133);});casper.waitFor(function() {
	return casper.evaluate(function() {
	...
	casper.exit(134);});casper.waitFor(function() {
	return casper.evaluate(function() {
		return ($('#promo-code').length ?  ($('#promo-code').size() > 0) :  true);
	});
}, undefined, function(){
	casper.exit(135);});casper.run(function() {
	casper.exit(0);
});